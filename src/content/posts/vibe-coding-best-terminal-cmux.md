...
它不替你设计工作流，不强迫你用某种固定的方式组织 agent，它只把基础设施做好——终端、浏览器、通知、工作区、分屏、CLI——然后让你自己决定怎么组合。这个思路正好和 vibe coding 的精神契合：工具本身要足够透明，让你可以按自己的节奏去构建。

---

## 真实用了多久，感受怎么样

我已经把 cmux 设为默认终端大概两周了。结论是：**回不去了**。

最直观的变化是"被动等待"变成了"主动感知"。以前我得时不时瞄一眼各个终端窗口，看 agent 有没有卡住，现在完全不需要，有通知 cmux 告诉我，没通知我就做别的。多个 agent 并行的时候效率提升是实打实的。

内置浏览器虽然不是我最常用的功能，但几次需要 agent 操作网页的时候，不用单独折腾配置这件事本身就值了。

侧边栏的 git 分支和 PR 信息对我来说也挺实用，尤其是多项目切换的时候，几乎消除了"我现在在哪个分支来着"的困惑。

---

## 总结

如果你在做 vibe coding，同时跑多个 Claude Code / Codex / 其他 agent，cmux 是目前我见过最对口的终端工具。可能不是功能最多的那个，但是**最懂 AI agent workflow 的那个**。

免费，开源（AGPL-3.0），macOS 原生(目前暂不支持Windows，官方还在适配中)，性能好，上手几乎没有成本，强烈推荐，直接去装！
