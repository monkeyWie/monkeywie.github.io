...
之前把官网从 `Vercel` 搬到 `Cloudflare Workers` 之后，有些用户反馈访问变慢了，甚至还有用户直接指出可以通过 `Cloudflare IP 优选` 来改善国内访问质量，试了一下，结果还挺不错。
...
到域名的 DNS 管理处（注意不是 Cloudflare 的 DNS，而是域名本身的 DNS 服务商），把业务域名的解析从原来的 A 记录改为 CNAME，指向前面提到的优选域名，记得不要开启小黄云。
